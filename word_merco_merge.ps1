...
param(
    ...
    [string]$InputPath = "C:\Users\user\OneDrive\Documents\Work_Files\Word Doc to database\H&H MASTER TESTING TEMPLATE - FOR REVIEW.dotx",
   
    [Parameter(Mandatory=$false)]
    [string]$OutputPath = "",
   
    [Parameter(Mandatory=$false)]
    [string]$TemplatePath = "C:\Users\user\OneDrive\Documents\Work_Files\Word Doc to database\H&H MASTER TESTING TEMPLATE - FOR REVIEW.dotx",
...
$script:DefaultTemplatePath = Join-Path $script:ScriptDirectory "H&H MASTER TESTING TEMPLATE - FOR REVIEW.dotx"
...
# Function to get current project number and bridge ID from database
function Get-CurrentProjectInfo {
    try {
        Write-ColorOutput "Retrieving current project information from database..." "Yellow"
        Write-LogFile -Message "Retrieving current project information from database..." -Level "INFO"
        
        # Verify database connection string is valid
        if ([string]::IsNullOrEmpty($script:ConnectionString)) {
            Write-ColorOutput "Database connection string is empty or invalid" "Red"
            Write-LogFile -Message "Database connection string is empty or invalid" -Level "ERROR"
            return $null
        }
        
        # Try to establish database connection with timeout
        $connectionTimeout = New-TimeSpan -Seconds 10
        $startTime = Get-Date
        $connection = $null
        ...
        try {
            ...
            Write-ColorOutput "Database connection failed: $($_.Exception.Message)" "Red"
            Write-LogFile -Message "Database connection failed: $($_.Exception.Message)" -Level "ERROR"
            return $null
        }
        
        if ($null -eq $connection) {
            Write-ColorOutput "Could not establish database connection" "Red"
            Write-LogFile -Message "Could not establish database connection" -Level "ERROR"
            return $null
        }
        
        # Execute query with error handling
        try {
            $query = "SELECT [project_number], [bridge_id], [ReportGeneratedTimeStamp] FROM [dbo].[CurrentProjectReport]"
            $recordset = $connection.Execute($query)
            
            if (-not $recordset.EOF) {
                $projectInfo = @{
                    ProjectNumber = $recordset.Fields("project_number").Value
                    BridgeId = $recordset.Fields("bridge_id").Value
                    Timestamp = $recordset.Fields("ReportGeneratedTimeStamp").Value
                }
                $recordset.Close()
                $connection.Close()
                
                Write-ColorOutput "Retrieved current project: $($projectInfo.ProjectNumber), bridge: $($projectInfo.BridgeId)" "Green"
                Write-LogFile -Message "Retrieved current project: $($projectInfo.ProjectNumber), bridge: $($projectInfo.BridgeId)" -Level "INFO"
                return $projectInfo
            } else {
                Write-ColorOutput "No current project found in database" "Yellow"
                Write-LogFile -Message "No current project found in database" -Level "WARN"
                $recordset.Close()
                $connection.Close()
                return $null
            }
        } catch {
            Write-ColorOutput "Error executing database query: $($_.Exception.Message)" "Red"
            Write-LogFile -Message "Error executing database query: $($_.Exception.Message)" -Level "ERROR"
            if ($connection) {
        ...
        Write-ColorOutput "Error retrieving current project info: $($_.Exception.Message)" "Red"
        Write-LogFile -Message "Error retrieving current project info: $($_.Exception.Message)" -Level "ERROR"
        return $null
    }
}
...
function Highlight-UnmergedPlaceholders {
    ...
    try {
        ...
        # First, search for NULL values and highlight them
        try {
            Write-ColorOutput "Searching for NULL values..." "Yellow"
            
            $range = $Document.Range()
            $find = $range.Find
            
            # Configure Find for NULL values
            ...
            $find.Forward = $true
            $find.Wrap = 0  # wdFindStop - Don't wrap around to prevent infinite loops
            
            $nullCount = 0
            $maxNullsToProcess = 100  # Safety limit to prevent infinite loops
            
            # Execute search for NULL values
            while ($find.Execute() -and $nullCount -lt $maxNullsToProcess) {
                $nullCount++
                $range = $find.Parent
                
                try {
                    # Apply yellow highlighting to NULL values
                    $range.HighlightColorIndex = $script:YellowHighlightColorIndex
                    $highlightedCount++
                    # Only log every 10th NULL value to reduce console spam
                    if ($nullCount % 10 -eq 0) {
                        Write-ColorOutput "Highlighted $nullCount NULL values so far..." "Yellow"
                    }
                } catch {
                    Write-ColorOutput "Error highlighting NULL value: $($_.Exception.Message)" "Red"
                }
            ...
            Write-ColorOutput "Found and highlighted $nullCount NULL values" "Green"
            if ($nullCount -ge $maxNullsToProcess) {
                Write-ColorOutput "WARNING: Reached maximum NULL value limit ($maxNullsToProcess). Some NULL values may not be highlighted." "Yellow"
            }
        } catch {
            Write-ColorOutput "Error searching for NULL values: $($_.Exception.Message)" "Red"
        }
        ...
        $guillemets = @(
            ...
            # @{ Open = '<<'; Close = '>>'; Name = "Double Angle Brackets" }  # Commented out as requested
        )
        ...
        foreach ($guillemet in $guillemets) {
            try {
                ...
                # Search for the specific guillemet pattern - more specific to avoid false positives
                ...
                $searchCount = 0
                $foundCount = 0
                while ($find.Execute() -and $searchCount -lt 500) {  # Increased safety limit from 100 to 500
                    $searchCount++
                    if ($find.Found) {
                        try {
                            $foundText = $range.Text
                            
                            # Improved validation for field placeholders
                            $isValidPlaceholder = $false
                            
                            # Stricter pattern matching to avoid highlighting non-placeholders
                            # Extract the content between the guillemets
                            $placeholderContent = $foundText.Substring($guillemet.Open.Length, $foundText.Length - $guillemet.Open.Length - $guillemet.Close.Length)
                            
                            # Skip if content contains spaces (not a valid merge field)
                            if ($placeholderContent -match "\s+") {
                                $isValidPlaceholder = $false
                            }
                            # Skip very short content that's likely not a field
                            elseif ($placeholderContent.Length -lt 2) {
                                $isValidPlaceholder = $false
                            }
                            # Skip if content is too long (likely not a field)
                            elseif ($placeholderContent.Length > 50) {
                                $isValidPlaceholder = $false
                            }
                            # Check if it's a valid field name pattern (letters, numbers, underscores)
                            elseif ($placeholderContent -match "^[a-zA-Z][a-zA-Z0-9_]*$") {
                                $isValidPlaceholder = $true
                            }
                            # Check for field names with underscores (common in merge fields)
                            elseif ($placeholderContent -match "^[a-zA-Z][a-zA-Z0-9]*(_[a-zA-Z0-9]+)+$") {
                                $isValidPlaceholder = $true
                            }
                            # Additional check for common merge field patterns
                            elseif ($placeholderContent -match "^(location|date|project|bridge|elevation|structure|foundation|drainage)_[a-zA-Z0-9_]+$") {
                                $isValidPlaceholder = $true
                            }
                            ...
                            if ($isValidPlaceholder) {
                                ...
                                $highlightedCount++
                                $foundCount++
                                Write-ColorOutput "Highlighted $($guillemet.Name) placeholder: '$foundText'" "Magenta"
                            } else {
                                Write-ColorOutput "Skipped non-placeholder: '$foundText'" "DarkGray"
                            }
                        } catch {
        ...
        try {
            # Removed output message to prevent showing in terminal
                    ...
                    if ($isOpenGuillemet) {
                        ...
                        if ($closePos -gt $i) {
                            try {
                                ...
                                # Extract content between guillemets
                            $placeholderContent = ""
                            if ($closeChar -eq '»') {
                                $placeholderContent = $placeholderText.Substring(1, $placeholderText.Length - 2)
                            } else {
                                $placeholderContent = $placeholderText.Substring(2, $placeholderText.Length - 4)
                            }
                            
                            # Apply the same validation as the main method
                            $isValid = $placeholderContent.Length -ge 2 -and
                                      $placeholderContent.Length -le 50 -and 
                                      !($placeholderContent -match "\s+") -and 
                                      ($placeholderContent -match "^[a-zA-Z][a-zA-Z0-9_]*$")
                                      $placeholderContent.Length -le 50 -and
                                      -not ($placeholderContent -match "\s+") -and
                                      ($placeholderContent -match "^[a-zA-Z][a-zA-Z0-9_]*$")
                            
                            if ($isValid) {
                                $placeholderRange.HighlightColorIndex = $script:OrangeHighlightColorIndex
                                $highlightedCount++
                                Write-ColorOutput "Manual search highlighted: '$placeholderText'" "Magenta"
                                }
                            } catch {
...
try {
    ...
    # Try to get project number and bridge ID from database first
    try {
        $currentProjectInfo = Get-CurrentProjectInfo
        if ($currentProjectInfo) {
            $ProjectNumber = $currentProjectInfo.ProjectNumber
            $BridgeId = $currentProjectInfo.BridgeId
            Write-ColorOutput "Using database values - Project: $ProjectNumber, Bridge: $BridgeId" "Green"
            Write-LogFile -Message "Using database values - Project: $ProjectNumber, Bridge: $BridgeId" -Level "INFO"
        } else {
            Write-ColorOutput "No current project found in database, using parameters or prompting user" "Yellow"
            Write-LogFile -Message "No current project found in database, using parameters or prompting user" -Level "WARN"
        }
    } catch {
        Write-ColorOutput "Error retrieving project info from database: $($_.Exception.Message)" "Red"
        Write-LogFile -Message "Error retrieving project info from database: $($_.Exception.Message)" -Level "ERROR"
    }
