...
param(
    ...
    [Parameter(Mandatory=$false)]
    [string]$OutputPath = "C:\Users\user\OneDrive\Documents\Work_Files\Word Doc to database",
...
function Get-ProjectNumbers {
    try {
        ...
        $recordset = $connection.Execute("SELECT DISTINCT project_number FROM [dbo].[vwHandHReportFormFields] ORDER BY project_number")
...
function Get-BridgeIds {
    ...
    try {
        ...
        $connection = Get-DatabaseConnection
        $query = "SELECT DISTINCT bridge_id FROM [dbo].[vwHandHReportFormFields] WHERE project_number = '$ProjectNumber' ORDER BY bridge_id"
...
try {
    ...
    if ($InputPath -eq "") {
        if (Test-Path $TemplatePath) {
            ...
            Write-ColorOutput "Please ensure HandHPS.dotm exists in the current directory." "Yellow"
            Write-ColorOutput "Usage: .\word_highlight_processor.ps1 -ProjectNumber 'H-025-999-25' -BridgeId 'K160.0'" "White"
            exit 1
        }
    }
    ...
    if ($TemplatePath -like "*HandHPS.dotm") {
